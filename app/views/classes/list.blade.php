...
@section('content')
	...
	<div class="page-header">
		<a title="Agregar Clase" class="btn btn-xs btn-default pull-right" href="{{ URL::to('classes/create') }}"><span class="glyphicon glyphicon-plus"></span></a>
  		<h1>{{ str_replace("/","-",substr($parent->url, 1)); }} <small>{{ $parent->description }}</small></h1>
	</div>
	<ol class="breadcrumb">
		<li><a href="{{ URL::to('classes') }}">Todas</a></li>
		{{ $bcurl = '' }}
		@foreach (explode("/", substr($parent->url, 1)) as $bcitem)
			<li>
				<a href="{{ URL::to('classes'.($bcurl .= ('/'. $bcitem)).'/list') }}">{{ $bcitem }}</a>
			</li>
		@endforeach
	</ol>
	...
	<table class="table table-striped table-bordered">
		...
		@if($classes->count() == 0)
			<tr>
				<td colspan="5"><strong>No contiene clases derivadas.</strong></td>
			</tr>
		@endif
